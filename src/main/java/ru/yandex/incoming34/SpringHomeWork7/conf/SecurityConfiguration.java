...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.authorizeRequests().antMatchers("/api/pruducts/user/**")
		.hasRole("USER")
				.antMatchers("/api/pruducts/admin/**").hasRole("ADMIN")
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication().withUser("Winnie").password(passwordEncoder().encode("456")).roles("USER");
		auth.inMemoryAuthentication().withUser("Piglet").password(passwordEncoder().encode("123")).roles("ADMIN")
		;
